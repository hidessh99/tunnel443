...
cat /etc/wildydev21/xray-cache/cache-nya.json | jq '.inbounds[2].settings.clients += [{"id": "'${uuidnya}'","email": "'${Username}'","alterid": '"0"'}]' > /etc/wildydev21/xray-cache/cache-nya2.json
cat /etc/wildydev21/xray-cache/cache-nya2.json | jq '.inbounds[5].settings.clients += [{"id": "'${uuidnya}'","email": "'${Username}'","alterid": '"0"'}]' > /etc/wildydev21/config/xray/tls.json
printf "y\n" | cp /etc/wildydev21/config/xray/nontls.json /etc/wildydev21/xray-cache/cache2-nya.json
cat /etc/wildydev21/xray-cache/cache2-nya.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuidnya}'","email": "'${Username}'","alterid": '"0"'}]' > /etc/wildydev21/config/xray/nontls.json
echo -e "Vmess $Username $exp $uuidnya" >> /etc/wildydev21/xray-client.conf

cat > /etc/wildydev21/cache/vmess-tls-gun.tmp << END
{"add":"${domain}","aid":"0","host":"","id":"${uuidnya}","net":"grpc","path":"Vmess-GRPC","port":"${tls_port}","ps":"${Username}","scy":"none","sni":"","tls":"tls","type":"gun","v":"2"}
END

cat > /etc/wildydev21/cache/vmess-tls-ws.tmp << END
{"add":"${domain}","aid":"0","host":"","id":"${uuidnya}","net":"ws","path":"/vmess","port":"${tls_port}","ps":"${Username}","scy":"none","sni":"${domain}","tls":"tls","type":"","v":"2"}
END

cat > /etc/wildydev21/cache/vmess-nontls.tmp << END
{"add":"${domain}","aid":"0","host":"","id":"${uuidnya}","net":"ws","path":"/vmess","port":"${nontls_port}","ps":"${Username}","scy":"none","sni":"","tls":"","type":"","v":"2"}
END
