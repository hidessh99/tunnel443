...
cat /etc/wildydev21/xray-cache/cache-nya.json | jq 'del(.inbounds[2].settings.clients[] | select(.email == "'${client}'"))' > /etc/wildydev21/xray-cache/cache-nya2.json
cat /etc/wildydev21/xray-cache/cache-nya2.json | jq 'del(.inbounds[5].settings.clients[] | select(.email == "'${client}'"))' > /etc/wildydev21/config/xray/tls.json
printf "y\n" | cp /etc/wildydev21/config/xray/nontls.json /etc/wildydev21/xray-cache/cache2-nya.json
cat /etc/wildydev21/xray-cache/cache2-nya.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${client}'"))' > /etc/wildydev21/config/xray/nontls.json
